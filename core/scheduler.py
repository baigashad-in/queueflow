...
async def schedule_task(task_id: str, run_at_timestamp: float) -> None:
    ...
    return await redis_client.zadd(REDIS_KEY, {task_id: run_at_timestamp})
...
async def get_due_tasks() -> list[str]:
    ...
    return await redis_client.zrangebyscore(REDIS_KEY, "-inf", current_time)
...
async def remove_scheduled(task_id: str) -> None:
    ...
    return await redis_client.zrem(REDIS_KEY, task_id)